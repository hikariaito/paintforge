...
const SB_URL="https://nwbhfjdgphfkfqhhbcqh.supabase.co";
const SB_KEY="sb_publishable_gLbbB1Ay4K7KpPx2-KujVw_OWTNSr21";
...
async function getSB(){
  ...
  return new Promise(res=>{
    if(window.__supabase){_sb=window.__supabase;res(_sb);return;}
    const s=document.createElement("script");
    s.src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2/dist/umd/supabase.js";
    s.onload=()=>{try{_sb=window.supabase.createClient(SB_URL,SB_KEY);window.__supabase=_sb;}catch(e){} res(_sb);};
    s.onerror=()=>res(null);
...
class ErrorBoundary extends Component {
  ...
  render(){
    if(this.state.error) return React.createElement("div",
      {style:{background:"#0d0d0d",color:"#e83030",padding:20,fontFamily:"monospace",fontSize:13,minHeight:"100vh"}},
  ...
  return (
    <div style={{marginBottom:14,position:"relative"}}>
      <button onClick={()=>setOpen(o=>!o)}
        ...
        <span style={{fontSize:16}}>{selected?getIC().folder:getIC().folder}</span>
...
function ManualBarcodeEntry({allPaints,tools,consumables,onFound,onFoundTool,onFoundCon,onTeach,onAddNew,onAddCustom,addNewMode="shop"}){
  ...
  },[val,paints.length,(tools||[]).length,(consumables||[]).length]);
    ...
    <div>
      ...
      {result?.found&&result.tool&&(
        <div style={{marginTop:6,border:`1px solid ${T.green}`,padding:"10px 12px",background:"rgba(0,204,102,.05)",borderRadius:10}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10,marginTop:8}}>
            <div style={{width:32,height:32,background:T.surface,border:`1px solid ${T.border}`,flexShrink:0,borderRadius:6,display:"flex",alignItems:"center",justifyContent:"center",color:T.dim}}>{getIC().tool}</div>
            <div style={{flex:1}}>
            ...
            <>
              ...
              {(tools||[]).filter(t=>!result.code||t.name.toLowerCase().includes(val.toLowerCase())||true).slice(0,5).map(t=>(
                <button key={t.id} onClick={()=>{
                  ...
                  <span style={{color:T.dim}}>{getIC().tool}</span>
                  <div style={{flex:1}}>
          ...
          {toolResults.map(t=>(
            <button key={t.id}
              ...
              <div style={{width:32,height:32,borderRadius:7,background:T.surface,
                border:`1px solid ${T.border}`,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16}}>{getIC().tool}</div>
              <div style={{flex:1,minWidth:0}}>
    ...
    <Sheet title={p.name} badge="PAINT" onClose={onClose}
      ...
      <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:12,overflow:"hidden",marginBottom:14}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 16px",borderBottom:`1px solid ${T.border}`}}>
          <span style={{display:"flex"}}>{getIC().flask}</span>
        ...
        {/* Library stepper */}
        <div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 16px",borderBottom:`1px solid ${T.border}`}}>
          <span style={{display:"flex"}}>{getIC().box}</span>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 16px"}}>
          <span style={{display:"flex"}}>{getIC().cart}</span>
      ...
      {photos.length>0&&(
        <div style={{marginBottom:14}}>
          <div style={{color:T.dim,fontSize:10,fontWeight:700,letterSpacing:2,textTransform:"uppercase",marginBottom:8,display:"flex",alignItems:"center",gap:6}}>{getIC().camera} PHOTOS</div>
      ...
      <button onClick={()=>setEditMode(e=>!e)}
        style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"space-between",
          ...
          transition:"all .2s"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <span style={{display:"flex"}}>{getIC().edit}</span>
        ...
        <div style={{border:`1px solid ${T.orange}`,borderTop:"none",borderRadius:"0 0 12px 12px",
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:14}}>
            ...
            <label style={{width:80,height:80,border:`2px dashed ${T.border}`,borderRadius:10,
              ...
              <span style={{display:"flex",justifyContent:"center"}}>{getIC().camera}</span>
              <span>ADD</span>
...
function ToolDetailSheet({tool,ownedIds,ownedMap,setOwnedMap,setTools,tools,onClose,onAddToShop,onDelete,setLightbox,cartToast}){
  ...
  const toolIcon=tool.type==="Brush"||tool.type==="Airbrush"?getIC().brush:tool.type==="Primer"||tool.type==="Varnish"?getIC().primer:tool.type==="Masking Tape"?getIC().masking:getIC().tool;
...
function PaintTrackerApp(){
  ...
  useEffect(()=>{
    async function load(){
      const db=await getSB(); if(!db) return;
      try{
        await db.from("pf_users").upsert({id:UID},{onConflict:"id"});
        const [b,o,pr,cp,tl,cn,sh,pj]=await Promise.all([
          db.from("pf_barcodes").select("barcode,paint_id").eq("user_id",UID),
          db.from("pf_owned").select("paint_id,qty").eq("user_id",UID),
          db.from("pf_prices").select("paint_id,price").eq("user_id",UID),
          db.from("pf_custom_paints").select("id,data").eq("user_id",UID),
          db.from("pf_tools").select("id,data").eq("user_id",UID),
          db.from("pf_consumables").select("id,data").eq("user_id",UID),
          db.from("pf_shop_items").select("id,data").eq("user_id",UID),
          db.from("pf_projects").select("id,data").eq("user_id",UID),
        ]);
        if(b.data?.length) setBarcodeMap(Object.fromEntries(b.data.map(r=>[r.barcode,r.paint_id])));
        if(o.data?.length) setOwnedMap(Object.fromEntries(o.data.map(r=>[r.paint_id,r.qty])));
        if(pr.data?.length) setPaintPrices(Object.fromEntries(pr.data.map(r=>[r.paint_id,r.price])));
        if(cp.data?.length) setCustomPaints(cp.data.map(r=>r.data));
        if(tl.data?.length) setTools(tl.data.map(r=>r.data));
        if(cn.data?.length) setConsumables(cn.data.map(r=>r.data));
        if(sh.data?.length) setShopItems(sh.data.map(r=>r.data));
        if(pj.data?.length) setProjects(pj.data.map(r=>r.data));
      }catch(e){console.log("sync load error",e);}
    }
    load();
  },[]);
  async function dbSave(table,rows){const db=await getSB();if(db)try{await db.from(table).upsert(rows,{onConflict:rows[0]&&"barcode"in rows[0]?"barcode":"id"});}catch(e){}}
  useEffect(()=>{if(Object.keys(barcodeMap).length)dbSave("pf_barcodes",Object.entries(barcodeMap).map(([barcode,paint_id])=>({barcode,paint_id,user_id:UID})));},[barcodeMap]);
  useEffect(()=>{if(Object.keys(ownedMap).length)dbSave("pf_owned",Object.entries(ownedMap).map(([paint_id,qty])=>({paint_id,qty,user_id:UID})));},[ownedMap]);
  useEffect(()=>{if(Object.keys(paintPrices).length)dbSave("pf_prices",Object.entries(paintPrices).map(([paint_id,price])=>({paint_id,price,user_id:UID})));},[paintPrices]);
  useEffect(()=>{if(customPaints.length)dbSave("pf_custom_paints",customPaints.map(p=>({id:p.id,user_id:UID,data:p})));},[customPaints]);
  useEffect(()=>{if(tools.length)dbSave("pf_tools",tools.map(t=>({id:t.id,user_id:UID,data:t})));},[tools]);
  useEffect(()=>{if(consumables.length)dbSave("pf_consumables",consumables.map(c=>({id:c.id,user_id:UID,data:c})));},[consumables]);
  useEffect(()=>{if(shopItems.length)dbSave("pf_shop_items",shopItems.map(s=>({id:s.id,user_id:UID,data:s})));},[shopItems]);
  useEffect(()=>{if(projects.length)dbSave("pf_projects",projects.map(p=>({id:p.id,user_id:UID,data:p})));},[projects]);
    ...
    <div style={{minHeight:"100vh",background:T.bg,fontFamily:T.font,color:T.white,position:"relative"}}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    {(project.startedAt||project.createdAt)&&(
                      <div style={{display:"flex",alignItems:"center",gap:5,color:T.dim,fontSize:11,fontWeight:600,letterSpacing:.5,marginBottom:8}}>
                        <span style={{display:"flex",flexShrink:0}}>{getIC().calendar}</span>
                            ...
                            return (
                              <div key={p.id} className="item-card" style={{display:"flex",alignItems:"center",gap:10,background:T.card,border:`1px solid ${owned?brand.color:T.border}`,padding:"8px 12px",marginBottom:3,borderRadius:10}}>
                                ...
                                <button onClick={()=>{addPaintToShop(p);setCartAddedPaintId(p.id);setTimeout(()=>setCartAddedPaintId(null),2000);}} title="Add to shopping list"
                                  ...
                                  {cartAddedPaintId===p.id?"✓":getIC().cart}
                                </button>
                        ...
                        <div key={tool.id}
                          ...
                          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:tool.images?.length?10:0}}>
                            <button onClick={e=>{e.stopPropagation();setToolDetail(tool);}}
                              ...
                              {tool.type==="Brush"||tool.type==="Airbrush"?getIC().brush:tool.type==="Primer"||tool.type==="Varnish"?getIC().primer:tool.type==="Masking Tape"?getIC().masking:getIC().tool}
                            </button>
                            <button onClick={e=>{e.stopPropagation();setToolDetail(tool);}}
                          ...
                          {(tool.location||tool.purchaseDate||tool.purchaseFrom)&&(
                            <div style={{display:"flex",gap:12,flexWrap:"wrap",marginTop:6,paddingTop:6,borderTop:`1px solid ${T.border}`}}>
                              {tool.location&&<div style={{display:"flex",alignItems:"center",gap:4}}><span style={{display:"flex",color:T.dim}}>{getIC().location}</span><span style={{color:T.dim,fontSize:11}}>{tool.location}</span></div>}
                              {tool.purchaseFrom&&<div style={{display:"flex",alignItems:"center",gap:4}}><span style={{display:"flex",color:T.dim}}>{getIC().store}</span><span style={{color:T.dim,fontSize:11}}>{tool.purchaseFrom}</span></div>}
                              {tool.purchaseDate&&<div style={{display:"flex",alignItems:"center",gap:4}}><span style={{display:"flex",color:T.dim}}>{getIC().calendar}</span><span style={{color:T.dim,fontSize:11}}>{new Date(tool.purchaseDate).toLocaleDateString("en-GB",{day:"2-digit",month:"short",year:"numeric"})}</span></div>}
                              {tool.receipt&&<button onClick={()=>setLightbox({photos:[{id:"r",src:tool.receipt,name:"Receipt"}],index:0})}
                                style={{display:"flex",alignItems:"center",gap:4,background:"none",border:"none",cursor:"pointer",padding:0}}>
                                <span style={{display:"flex",color:T.orange}}>{getIC().receipt}</span><span style={{color:T.orange,fontSize:11,fontWeight:700}}>RECEIPT</span>
                              </button>}
                            </div>
                          )}
                        ...
                        return (
                          <div key={con.id}
                            ...
                            {(con.location||con.purchaseDate||con.purchaseFrom)&&(
                              <div style={{display:"flex",gap:12,flexWrap:"wrap",marginTop:6,paddingTop:6,borderTop:`1px solid ${T.border}`}}>
                                {con.location&&<div style={{display:"flex",alignItems:"center",gap:4}}><span style={{display:"flex",color:T.dim}}>{getIC().location}</span><span style={{color:T.dim,fontSize:11}}>{con.location}</span></div>}
                                {con.purchaseFrom&&<div style={{display:"flex",alignItems:"center",gap:4}}><span style={{display:"flex",color:T.dim}}>{getIC().store}</span><span style={{color:T.dim,fontSize:11}}>{con.purchaseFrom}</span></div>}
                                {con.purchaseDate&&<div style={{display:"flex",alignItems:"center",gap:4}}><span style={{display:"flex",color:T.dim}}>{getIC().calendar}</span><span style={{color:T.dim,fontSize:11}}>{new Date(con.purchaseDate).toLocaleDateString("en-GB",{day:"2-digit",month:"short",year:"numeric"})}</span></div>}
          ...
          <>
            ...
            {shopItems.length===0&&(
              <div style={{border:`2px dashed ${T.border}`,borderRadius:12,padding:40,textAlign:"center",color:T.dimmer,cursor:"pointer"}} onClick={()=>setShowAddShop(true)}>
                <div style={{fontSize:28,marginBottom:8,display:"flex",justifyContent:"center"}}>{getIC().cart}</div>
          ...
          {nTName.trim().length>=3&&(()=>{
            ...
            return (
              <div style={{marginBottom:10,border:`1px solid ${T.yellow}`,borderRadius:10,padding:"10px 12px",background:"rgba(255,208,0,.05)"}}>
                ...
                {matches.map(t=>{
                  const toolIcon=t.type==="Brush"||t.type==="Airbrush"?getIC().brush:t.type==="Primer"||t.type==="Varnish"?getIC().primer:t.type==="Masking Tape"?getIC().masking:getIC().tool;
                  return (
      ...
      {showBackup&&(
        <Sheet title="Backup & Restore" badge="DATA" onClose={()=>{setShowBackup(false);setImportError("");setImportSuccess(false);}}>
          {/* Export */}
          <div style={{marginBottom:20}}>
            <div style={{color:T.white,fontWeight:900,fontSize:13,marginBottom:6}}>EXPORT BACKUP</div>
