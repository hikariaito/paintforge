...
// Supabase — lazy loaded so app works even if it fails
...
function getUserId(){
  ...
  if(!id){id="user_"+Math.random().toString(36).slice(2)+Date.now().toString(36);localStorage.setItem("pf_user_id",id);}
  return id;
}
...
export default function PaintTracker(){
  ...
  useEffect(()=>{
    async function loadFromSupabase(){
      ...
      if(!db) return;
      try{
        await db.from("pf_users").upsert({id:USER_ID},{onConflict:"id"});
        const [barcodes,owned,prices,customs,toolsRes,consRes,shopRes,projRes]=await Promise.all([
          db.from("pf_barcodes").select("barcode,paint_id").eq("user_id",USER_ID),
          db.from("pf_owned").select("paint_id,qty").eq("user_id",USER_ID),
          db.from("pf_prices").select("paint_id,price").eq("user_id",USER_ID),
          db.from("pf_custom_paints").select("id,data").eq("user_id",USER_ID),
          db.from("pf_tools").select("id,data").eq("user_id",USER_ID),
          db.from("pf_consumables").select("id,data").eq("user_id",USER_ID),
          db.from("pf_shop_items").select("id,data").eq("user_id",USER_ID),
          db.from("pf_projects").select("id,data").eq("user_id",USER_ID),
        ]);
        if(barcodes.data?.length) setBarcodeMap(Object.fromEntries(barcodes.data.map(r=>[r.barcode,r.paint_id])));
        if(owned.data?.length) setOwnedMap(Object.fromEntries(owned.data.map(r=>[r.paint_id,r.qty])));
        if(prices.data?.length) setPaintPrices(Object.fromEntries(prices.data.map(r=>[r.paint_id,r.price])));
        if(customs.data?.length) setCustomPaints(customs.data.map(r=>r.data));
        if(toolsRes.data?.length) setTools(toolsRes.data.map(r=>r.data));
        if(consRes.data?.length) setConsumables(consRes.data.map(r=>r.data));
        if(shopRes.data?.length) setShopItems(shopRes.data.map(r=>r.data));
        if(projRes.data?.length) setProjects(projRes.data.map(r=>r.data));
  ...
  useEffect(()=>{
    ...
    sb.from("pf_barcodes").upsert(entries.map(([barcode,paint_id])=>({barcode,paint_id,user_id:USER_ID})),{onConflict:"barcode"}).then();
  },[barcodeMap]);
  useEffect(()=>{
    const entries=Object.entries(ownedMap);
    if(!entries.length) return;
    sb.from("pf_owned").upsert(entries.map(([paint_id,qty])=>({paint_id,qty,user_id:USER_ID})),{onConflict:"paint_id,user_id"}).then();
  },[ownedMap]);
  useEffect(()=>{
    const entries=Object.entries(paintPrices);
    if(!entries.length) return;
    sb.from("pf_prices").upsert(entries.map(([paint_id,price])=>({paint_id,price,user_id:USER_ID})),{onConflict:"paint_id,user_id"}).then();
  },[paintPrices]);
  useEffect(()=>{
    if(!customPaints.length) return;
    sb.from("pf_custom_paints").upsert(customPaints.map(p=>({id:p.id,user_id:USER_ID,data:p})),{onConflict:"id"}).then();
  },[customPaints]);
  useEffect(()=>{
    getSB().then(db=>{if(db) db.from("pf_tools").upsert(tools.map(t=>({id:t.id,user_id:USER_ID,data:t})),{onConflict:"id"}).then();});
  },[tools]);
  useEffect(()=>{
    getSB().then(db=>{if(db) db.from("pf_consumables").upsert(consumables.map(c=>({id:c.id,user_id:USER_ID,data:c})),{onConflict:"id"}).then();});
  },[consumables]);
  useEffect(()=>{
    if(!shopItems.length) return;
    sb.from("pf_shop_items").upsert(shopItems.map(s=>({id:s.id,user_id:USER_ID,data:s})),{onConflict:"id"}).then();
  },[shopItems]);
  useEffect(()=>{
    if(!projects.length) return;
    sb.from("pf_projects").upsert(projects.map(p=>({id:p.id,user_id:USER_ID,data:p})),{onConflict:"id"}).then();
  },[projects]);
