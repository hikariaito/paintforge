...
let _localVersion=0; // increments on every local change
let _remoteVersion=0; // last version we loaded from remote
...
function PaintTrackerApp(){
  ...
  useEffect(()=>{
    async function load(){
      // Skip if pending save or saved within last 10 seconds
      if(_pendingSave||Date.now()-_lastSaveTime<10000) return;
      setSyncStatus("loading");
      const versionAtStart=_localVersion;
      ...
      // Only apply if no local changes happened while we were loading
      if(_localVersion!==versionAtStart) return;
