...
import { syncLoad, syncSave, getUID } from "./sync.js";
...
function PaintTrackerApp(){
  ...
  // ── Supabase sync ──
  useEffect(()=>{
    syncLoad(UID,{setBarcodeMap,setOwnedMap,setPaintPrices,setCustomPaints,setTools,setConsumables,setShopItems,setProjects});
  },[]);
  useEffect(()=>{if(Object.keys(barcodeMap).length) syncSave("pf_barcodes",Object.entries(barcodeMap).map(([barcode,paint_id])=>({barcode,paint_id,user_id:UID})));},[barcodeMap]);
  useEffect(()=>{if(Object.keys(ownedMap).length) syncSave("pf_owned",Object.entries(ownedMap).map(([paint_id,qty])=>({paint_id,qty,user_id:UID})));},[ownedMap]);
  useEffect(()=>{if(Object.keys(paintPrices).length) syncSave("pf_prices",Object.entries(paintPrices).map(([paint_id,price])=>({paint_id,price,user_id:UID})));},[paintPrices]);
  useEffect(()=>{if(customPaints.length) syncSave("pf_custom_paints",customPaints.map(p=>({id:p.id,user_id:UID,data:p})));},[customPaints]);
  useEffect(()=>{if(tools.length) syncSave("pf_tools",tools.map(t=>({id:t.id,user_id:UID,data:t})));},[tools]);
  useEffect(()=>{if(consumables.length) syncSave("pf_consumables",consumables.map(c=>({id:c.id,user_id:UID,data:c})));},[consumables]);
  useEffect(()=>{if(shopItems.length) syncSave("pf_shop_items",shopItems.map(s=>({id:s.id,user_id:UID,data:s})));},[shopItems]);
  useEffect(()=>{if(projects.length) syncSave("pf_projects",projects.map(p=>({id:p.id,user_id:UID,data:p})));},[projects]);
