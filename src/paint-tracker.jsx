...
function BarcodeScanner({allPaints,onFound,onTeach,onClose,onAddNew,addNewMode="shop"}){
  ...
  async function decodeImage(file){
    ...
    try{
      ...
      if(!Z){setStatus("photo_fail");setDecoding(false);return;}
      const hints=new Map();
      ...
      for(const scale of [1,0.75,0.5,1.25]){
        try{
          ...
          if(result){handleCode(result.getText());found=true;break;}
        }catch(e){}
      }
      URL.revokeObjectURL(url);
      if(!found) setStatus("photo_fail");
    ...
    <div style={{border:`1px solid ${T.border}`,background:"#0a0a0a",padding:12,borderRadius:10}}>
      ...
      {status==="photo_fail"&&(
        <div style={{padding:"8px 0"}}>
          ...
          <div style={{color:T.dim,fontSize:12,margin:"8px 0"}}>Try again — hold the camera closer and make sure the barcode is flat and well lit.</div>
