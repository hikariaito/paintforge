...
// Supabase client — initialised once, used throughout
...
export default function PaintTracker(){
  ...
  useEffect(()=>{
    async function loadFromSupabase(){
      if(!sb) return;
      try{
        await sb.from("pf_users").upsert({id:USER_ID},{onConflict:"id"});
        const [barcodes,owned,prices,customs,toolsRes,consRes,shopRes,projRes]=await Promise.all([
          sb.from("pf_barcodes").select("barcode,paint_id").eq("user_id",USER_ID),
          sb.from("pf_owned").select("paint_id,qty").eq("user_id",USER_ID),
          sb.from("pf_prices").select("paint_id,price").eq("user_id",USER_ID),
          sb.from("pf_custom_paints").select("id,data").eq("user_id",USER_ID),
          sb.from("pf_tools").select("id,data").eq("user_id",USER_ID),
          sb.from("pf_consumables").select("id,data").eq("user_id",USER_ID),
          sb.from("pf_shop_items").select("id,data").eq("user_id",USER_ID),
          sb.from("pf_projects").select("id,data").eq("user_id",USER_ID),
        ]);
  ...
  useEffect(()=>{
    sb.from("pf_tools").upsert(tools.map(t=>({id:t.id,user_id:USER_ID,data:t})),{onConflict:"id"}).then();
  },[tools]);
  useEffect(()=>{
    sb.from("pf_consumables").upsert(consumables.map(c=>({id:c.id,user_id:USER_ID,data:c})),{onConflict:"id"}).then();
  },[consumables]);
  useEffect(()=>{
