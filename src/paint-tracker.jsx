...
async function sbUpsert(table,rows){
  ...
  try{
    const r=await fetch(`${_SB_URL}/${table}`,{method:"POST",headers:{"apikey":_SB_KEY,"Authorization":"Bearer "+_SB_KEY,"Content-Type":"application/json","Prefer":"resolution=merge-duplicates"},body:JSON.stringify(rows)});
    const text=await r.text();
    if(!r.ok){_syncError=`POST ${table}: ${r.status} ${text.slice(0,80)}`;_syncSetStatus&&_syncSetStatus("error:"+_syncError);}
    else _syncSetStatus&&_syncSetStatus("ok");
    ...
    <div style={{minHeight:"100vh",background:T.bg,fontFamily:T.font,color:T.white,position:"relative"}}>
      ...
      <div style={{background:T.bg,padding:"10px 16px",
        ...
        <div style={{display:"flex",alignItems:"center",gap:8,position:"relative",zIndex:1}}>
          ...
          {/* Sync status dot */}
          <div title={syncStatus.startsWith("error:")?syncStatus.slice(6):"Sync: "+syncStatus}
            style={{width:8,height:8,borderRadius:"50%",flexShrink:0,cursor:"pointer",
              background:syncStatus==="ok"?"#00cc66":syncStatus==="loading"?T.orange:syncStatus.startsWith("error:")?T.red:T.border}}
            onClick={()=>syncStatus.startsWith("error:")&&alert("Sync error:\n"+syncStatus.slice(6))}/>
