...
async function sbUpsert(table,rows,conflictCol="id"){
  ...
  try{
    ...
    else{_lastSaveTime=Date.now();_syncSetStatus&&_syncSetStatus("ok");}
...
function ManualBarcodeEntry({allPaints,tools,consumables,onFound,onFoundTool,onFoundCon,onTeach,onAddNew,onAddCustom,addNewMode="shop"}){
  ...
  useEffect(()=>{
    ...
    // Name/id match — return ALL matches
    const q=clean.toLowerCase();
    const nameMatches=paints.filter(p=>p.name.toLowerCase().includes(q)||p.id.toLowerCase().includes(q));
...
function PaintTrackerApp(){
  ...
  useEffect(()=>{
    async function load(){
      // Skip if we just saved — avoids overwriting local changes
    ...
    // Supabase Realtime via WebSocket — instant updates, no polling
    const wsUrl=`wss://hnnyfxtubahbxuvqgctm.supabase.co/realtime/v1/websocket?apikey=${_SB_KEY}&vsn=1.0.0`;
    let ws,pingInterval;
    function connectWS(){
      try{
        ...
        ws.onopen=()=>{
          // Join all pf_ tables
          ws.send(JSON.stringify({topic:`realtime:public`,event:"phx_join",payload:{config:{broadcast:{self:false},presence:{key:""},postgres_changes:[
            {event:"*",schema:"public",table:"pf_owned"},
            {event:"*",schema:"public",table:"pf_barcodes"},
            {event:"*",schema:"public",table:"pf_prices"},
            {event:"*",schema:"public",table:"pf_tools"},
            {event:"*",schema:"public",table:"pf_consumables"},
            {event:"*",schema:"public",table:"pf_shop_items"},
            {event:"*",schema:"public",table:"pf_projects"},
          ]}},ref:"1"}));
          pingInterval=setInterval(()=>ws.readyState===1&&ws.send(JSON.stringify({topic:"phoenix",event:"heartbeat",payload:{},ref:"ping"})),20000);
        };
        ws.onmessage=e=>{
          try{
            ...
            if(msg.event==="postgres_changes"&&msg.payload?.data?.record){
              ...
              // Only update if change is from same uid
              if(record.user_id===uid) load();
            }
          }catch{}
        };
        ws.onclose=()=>{clearInterval(pingInterval);setTimeout(connectWS,3000);}; // reconnect
        ws.onerror=()=>ws.close();
      }catch(e){}
    }
    connectWS();
    // Fallback poll every 10s in case WS fails
    const interval=setInterval(load,10000);
    return ()=>{clearInterval(interval);clearInterval(pingInterval);try{ws&&ws.close();}catch{}};
  },[]);
  useEffect(()=>{if(Object.keys(barcodeMap).length) sbUpsert("pf_barcodes",Object.entries(barcodeMap).map(([barcode,paint_id])=>({barcode,paint_id,user_id:uid})),"barcode");},[barcodeMap]);
