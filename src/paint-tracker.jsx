...
function BarcodeScanner({allPaints,onFound,onTeach,onClose,onAddNew,addNewMode="shop"}){
  ...
  async function startLiveScan(){
    if(!("BarcodeDetector" in window)){
      setCamError("Live scan requires iOS 17+ or Chrome on desktop. Use the camera photo option below instead.");
      setStatus("cam_error");
      return;
    }
    setStatus("starting");
    ...
    <div style={{border:`1px solid ${T.border}`,background:"#0a0a0a",padding:12,borderRadius:10}}>
      ...
      {status==="cam_error"&&(
        <div style={{padding:"8px 0",textAlign:"center"}}>
          ...
          <Btn onClick={()=>setStatus("idle")} small>USE PHOTO INSTEAD</Btn>
        </div>
      )}
