...
// Supabase REST API — no library, no bundler conflicts
const SB="https://nwbhfjdgphfkfqhhbcqh.supabase.co/rest/v1";
...
async function sbGet(table,uid){
  try{const r=await fetch(`${SB}/${table}?user_id=eq.${uid}`,{headers:SBH});return r.ok?await r.json():[];}catch{return [];}
}
async function sbUpsert(table,rows){
  if(!rows.length) return;
  try{await fetch(`${SB}/${table}`,{method:"POST",headers:{...SBH,"Prefer":"resolution=merge-duplicates"},body:JSON.stringify(rows)});}catch{}
}
async function sbEnsureUser(uid){
  try{await fetch(`${SB}/pf_users`,{method:"POST",headers:{...SBH,"Prefer":"resolution=merge-duplicates"},body:JSON.stringify([{id:uid}])});}catch{}
}
...
function PaintTrackerApp(){
  ...
  useEffect(()=>{if(Object.keys(barcodeMap).length) sbUpsert("pf_barcodes",Object.entries(barcodeMap).map(([barcode,paint_id])=>({barcode,paint_id,user_id:uid})));},[barcodeMap]);
  useEffect(()=>{if(Object.keys(ownedMap).length) sbUpsert("pf_owned",Object.entries(ownedMap).map(([paint_id,qty])=>({paint_id,qty,user_id:uid})));},[ownedMap]);
  useEffect(()=>{if(Object.keys(paintPrices).length) sbUpsert("pf_prices",Object.entries(paintPrices).map(([paint_id,price])=>({paint_id,price,user_id:uid})));},[paintPrices]);
  useEffect(()=>{if(customPaints.length) sbUpsert("pf_custom_paints",customPaints.map(p=>({id:p.id,user_id:uid,data:p})));},[customPaints]);
  useEffect(()=>{if(tools.length) sbUpsert("pf_tools",tools.map(t=>({id:t.id,user_id:uid,data:t})));},[tools]);
  useEffect(()=>{if(consumables.length) sbUpsert("pf_consumables",consumables.map(c=>({id:c.id,user_id:uid,data:c})));},[consumables]);
  useEffect(()=>{if(shopItems.length) sbUpsert("pf_shop_items",shopItems.map(s=>({id:s.id,user_id:uid,data:s})));},[shopItems]);
  useEffect(()=>{if(projects.length) sbUpsert("pf_projects",projects.map(p=>({id:p.id,user_id:uid,data:p})));},[projects]);
