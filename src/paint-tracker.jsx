...
async function sbUpsert(table,rows,conflictCol="id"){
  ...
  try{
    ...
    else{_lastSaveTime=Date.now();_pendingSave=false;_syncSetStatus&&_syncSetStatus("ok");setTimeout(()=>_syncSetStatus&&_syncSetStatus("idle"),3000);}
...
function PaintTrackerApp(){
  ...
  useEffect(()=>{
    async function load(){
      // Skip if we recently saved — avoids overwriting local changes
      if(_pendingSave||Date.now()-_lastSaveTime<8000) return;
