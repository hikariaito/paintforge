...
const sb=createClient("https://nwbhfjdgphfkfqhhbcqh.supabase.co","sb_publishable_gLbbB1Ay4K7KpPx2-KujVw_OWTNSr21");
function getUserId(){let id=localStorage.getItem("pf_uid");if(!id){id="u"+Math.random().toString(36).slice(2);localStorage.setItem("pf_uid",id);}return id;}
const UID=getUserId();
...
export default function PaintTracker(){
  ...
  useEffect(()=>{
    ...
    sb.from("pf_barcodes").upsert(entries.map(([barcode,paint_id])=>({barcode,paint_id,user_id:UID})),{onConflict:"barcode"}).then();
  },[barcodeMap]);
  useEffect(()=>{
    const entries=Object.entries(ownedMap);
    if(!entries.length) return;
    sb.from("pf_owned").upsert(entries.map(([paint_id,qty])=>({paint_id,qty,user_id:UID})),{onConflict:"paint_id,user_id"}).then();
  },[ownedMap]);
  useEffect(()=>{
    const entries=Object.entries(paintPrices);
    if(!entries.length) return;
    sb.from("pf_prices").upsert(entries.map(([paint_id,price])=>({paint_id,price,user_id:UID})),{onConflict:"paint_id,user_id"}).then();
  },[paintPrices]);
  useEffect(()=>{
    if(!customPaints.length) return;
    sb.from("pf_custom_paints").upsert(customPaints.map(p=>({id:p.id,user_id:UID,data:p})),{onConflict:"id"}).then();
  },[customPaints]);
  useEffect(()=>{
    sb.from("pf_tools").upsert(tools.map(t=>({id:t.id,user_id:UID,data:t})),{onConflict:"id"}).then();
  },[tools]);
  useEffect(()=>{
    sb.from("pf_consumables").upsert(consumables.map(c=>({id:c.id,user_id:UID,data:c})),{onConflict:"id"}).then();
  },[consumables]);
  useEffect(()=>{
    if(!shopItems.length) return;
    sb.from("pf_shop_items").upsert(shopItems.map(s=>({id:s.id,user_id:UID,data:s})),{onConflict:"id"}).then();
  },[shopItems]);
  useEffect(()=>{
    if(!projects.length) return;
    sb.from("pf_projects").upsert(projects.map(p=>({id:p.id,user_id:UID,data:p})),{onConflict:"id"}).then();
  },[projects]);
