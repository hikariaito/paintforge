...
async function sbGet(table,uid){try{const r=await fetch("https://nwbhfjdgphfkfqhhbcqh.supabase.co/rest/v1/"+table+"?user_id=eq."+uid,{headers:{"apikey":"sb_publishable_COUPDWELnxj_QtPyFP76QA_cDP-4Kvv","Authorization":"Bearer sb_publishable_COUPDWELnxj_QtPyFP76QA_cDP-4Kvv"}});return r.ok?await r.json():[];}catch{return [];}}
async function sbUpsert(table,rows){if(!rows.length)return;try{await fetch("https://nwbhfjdgphfkfqhhbcqh.supabase.co/rest/v1/"+table,{method:"POST",headers:{"apikey":"sb_publishable_COUPDWELnxj_QtPyFP76QA_cDP-4Kvv","Authorization":"Bearer sb_publishable_COUPDWELnxj_QtPyFP76QA_cDP-4Kvv","Content-Type":"application/json","Prefer":"resolution=merge-duplicates"},body:JSON.stringify(rows)});}catch{}}
