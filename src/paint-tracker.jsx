  ...
  useEffect(()=>{
    ...
    async function start(){
      try{
        ...
        if(active){
          setStatus("error");
          const msg=e?.name==="NotAllowedError"?"Permission denied — tap Allow when asked for camera access."
            :e?.name==="NotFoundError"?"No camera found on this device."
            :e?.name==="NotReadableError"?"Camera is in use by another app — close other apps and try again."
            :e?.name==="OverconstrainedError"?"Camera constraint error — trying fallback."
            :`${e?.name||"Error"}: ${e?.message||"Unknown"}`;
          setCamError(msg);
          // If environment camera failed, retry with any camera
          if(e?.name==="OverconstrainedError"){
            try{
              await navigator.mediaDevices.getUserMedia({video:true});
              setCamError("Try switching to Photo Scan — environment camera unavailable.");
            }catch(e2){
              setCamError(`${e2?.name}: ${e2?.message}`);
            }
          }
        }
      }
...
export default function PaintTracker(){
  ...
  const [paintPrices,setPaintPrices]=useState({}); // paintId -> price for any paint
    ...
    <div style={{minHeight:"100vh",background:T.bg,fontFamily:T.font,color:T.white,position:"relative"}}>
      ...
      {showAddTool&&(
        <Sheet title="Add Tool" badge="TOOL" onClose={()=>{setShowAddTool(false);setNTScan(false);setToolTopScan(false);}}
          ...
          <Field value={nTPurchaseFrom} onChange={setNTPurchaseFrom} placeholder="E.G. AMAZON, LOCAL HOBBY SHOP"/> onChange={e=>setNTPurchaseDate(e.target.value)}
            style={{width:"100%",maxWidth:"100%",boxSizing:"border-box",background:"#0a0a0a",border:`1px solid ${T.border}`,
      ...
      {showAddCon&&(
        <Sheet title="Add Supply Item" badge="SUPPLIES" onClose={()=>{setShowAddCon(false);setNCnScan(false);setConTopScan(false);}}
          ...
          <FieldLabel>Where you got it from</FieldLabel><Field value={nCnPurchaseFrom} onChange={setNCnPurchaseFrom} placeholder="E.G. AMAZON, LOCAL HOBBY SHOP"/> onChange={e=>setNCnPurchaseDate(e.target.value)}
            style={{width:"100%",maxWidth:"100%",boxSizing:"border-box",background:"#0a0a0a",border:`1px solid ${T.border}`,
              padding:"6px 10px",color:T.white,fontSize:12,fontFamily:T.font,outline:"none",marginBottom:10,colorScheme:"dark",display:"block"}}/>
          <FieldLabel>Receipt photo (optional)</FieldLabel>
          {nCnReceipt?(
                ...
                <div style={{marginBottom:12}}>
                  <BarcodeScanner
                    ...
                    onFound={p=>{
                      ...
                      setTimeout(()=>{setConfirmAddToShop({name:p.name,brand:BRANDS[p.brand]?.name||p.brand,category:"Paint",barcode:p.barcode||"",hex:p.hex});setAddShopQty(1);setAddShopNotes("");setAddShopPrice("");},50);
                    }}
                    onTeach={code=>{setShopTeach(code);setShopScan(false);}}
