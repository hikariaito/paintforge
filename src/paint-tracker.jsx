...
async function sbGet(table,uid){try{const r=await fetch("https://hnnyfxtubahbxuvqgctm.supabase.co/rest/v1/"+table+"?user_id=eq."+uid,{headers:{"apikey":_SB_KEY,"Authorization":"Bearer "+_SB_KEY}});return r.ok?await r.json():[];}catch{return [];}}
async function sbUpsert(table,rows){if(!rows.length)return;try{await fetch("https://hnnyfxtubahbxuvqgctm.supabase.co/rest/v1/"+table,{method:"POST",headers:{"apikey":_SB_KEY,"Authorization":"Bearer "+_SB_KEY,"Content-Type":"application/json","Prefer":"resolution=merge-duplicates"},body:JSON.stringify(rows)});}catch{}}
