...
const ALL_PAINTS = [
  ...
  { id:"KV203", brand:"kaleido", line:"Kaleido Varnish", name:"Varnish Matt",     hex:"#d8e8f0", barcode:"06975834948573" },
  // ── Mecha Washes ──
...
function BarcodeScanner({allPaints,onFound,onTeach,onClose,onAddNew,addNewMode="shop"}){
  ...
  function handleCode(code){
    setScannedCode(code);
    const match=paints.find(p=>p.barcode===code||(p.barcode&&p.barcode.replace(/^0/,"")===code.replace(/^0/,"")));
  ...
  async function decodeImage(file){
    ...
    // Convert to base64 so it persists in state reliably
    ...
    // Apply EXIF orientation so image is always upright before decoding
    async function correctOrientation(b64){
    ...
    const correctedBase64=await correctOrientation(base64);

    try{
      let foundCode=null;

      // Stage 1 — try native BarcodeDetector (Safari iOS 17+)
      if("BarcodeDetector" in window){
        try{
          const bd=new window.BarcodeDetector({formats:["ean_13","ean_8","upc_a","upc_e","code_128","code_39"]});
          const img=new Image(); img.src=correctedBase64;
      ...
      // Stage 2 — ZXing at multiple scales with preprocessing
      if(!foundCode){
        ...
        if(Z){
          ...
          const img=new Image(); img.src=correctedBase64;
          await new Promise(r=>{img.onload=r;img.onerror=r;});
          // Try multiple scales AND a high-contrast greyscale version
          const variants=[];
          for(const scale of [1,1.5,0.75,2]){
            ...
            for(let i=0;i<d.data.length;i+=4){
              const grey=0.299*d.data[i]+0.587*d.data[i+1]+0.114*d.data[i+2];
              const v=grey>128?255:0; // threshold to pure black/white
          ...
          for(const canvas of variants){
            try{
              ...
              if(result){foundCode=result.getText();break;}
            }catch(e){}
          }
        }
      }

      // Stage 3 — crop to middle third (where barcode usually is) and retry with rotations
      if(!foundCode){
        const Z=window.ZXing;
        if(Z){
          const hints=new Map();
          ...
          function makeCanvas(srcImg,rotation,scale,contrast){
          ...
          // Try all 4 rotations × normal + contrast
          outer:
          for(const rotation of [0,90,180,270]){
            for(const contrast of [false,true]){
              for(const scale of [1,1.5]){
                try{
                  const c=makeCanvas(img,rotation,scale,contrast);
      ...
      if(foundCode){
        ...
        setFailedImage(base64);
        setStatus("photo_fail");
      }
    }catch(e){
      setFailedImage(base64);
      setStatus("photo_fail");
  ...
  return (
    <div style={{border:`1px solid ${T.border}`,background:"#0a0a0a",padding:12,borderRadius:10}}>
      ...
      {/* Mode buttons — CAMERA (native) and PHOTO SCAN */}
      {!isResult&&status!=="decoding"&&(
        <div style={{display:"flex",gap:8,marginBottom:12}}>
          {/* Native camera — works on iOS, triggers camera app directly */}
          <label style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",gap:6,
            padding:"10px 0",background:T.orange,color:"#000",fontWeight:900,fontSize:11,
            letterSpacing:1,textTransform:"uppercase",borderRadius:8,cursor:"pointer",border:"none"}}>
            ...
            SCAN WITH CAMERA
            <input ref={liveInputRef} type="file" accept="image/*" capture="environment"
              onChange={e=>decodeImage(e.target.files?.[0])}
              style={{display:"none"}}/>
          </label>
          {/* Photo from library */}
          <label style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",gap:6,
            padding:"10px 0",background:T.surface,color:T.dim,fontWeight:900,fontSize:11,
            letterSpacing:1,textTransform:"uppercase",borderRadius:8,cursor:"pointer",border:`1px solid ${T.border}`}}>
      ...
      {/* Decoding spinner */}
      {status==="decoding"&&(
        <div style={{padding:"20px 0",textAlign:"center",color:T.dim,fontSize:12,letterSpacing:1}}>
          READING BARCODE...
        </div>
      )}
      ...
      {status==="photo_fail"&&(
        <div style={{padding:"8px 0"}}>
          ...
          <div style={{color:T.dim,fontSize:12,margin:"8px 0"}}>Couldn't read the barcode automatically. You can type it below from the image.</div>
          {/* Show the photo so user can read the number */}
          {failedImage&&(
            <div style={{marginBottom:10,borderRadius:8,overflow:"hidden",border:`1px solid ${T.border}`,background:"#000",position:"relative"}}>
              <img src={failedImage} alt="scanned" style={{width:"100%",maxHeight:260,objectFit:"contain",display:"block"}}/>
              <div style={{position:"absolute",bottom:0,left:0,right:0,background:"rgba(0,0,0,.75)",
                ...
                Read the digits below the barcode lines and type them in
              </div>
            </div>
          )}
          <button onClick={()=>{setStatus("idle");setManualCode("");setFailedImage(null);}} style={{width:"100%",background:T.bg,border:`1px solid ${T.orange}`,
            color:T.orange,fontWeight:900,fontSize:11,letterSpacing:1,textTransform:"uppercase",padding:"9px 0",cursor:"pointer",marginBottom:12,borderRadius:8}}>
            TRY AGAIN
          </button>
          {/* Manual barcode entry — auto-searches as you type */}
          <div style={{marginBottom:10}}>
            <div style={{color:T.dim,fontSize:11,fontWeight:700,letterSpacing:1,textTransform:"uppercase",marginBottom:6}}>Type barcode number from photo:</div>
            <input value={manualCode} onChange={e=>{
              setManualCode(e.target.value);
              // Auto-match if it looks like a full barcode
              ...
              placeholder="E.G. 06975834948573"
              inputMode="numeric"
              style={{width:"100%",boxSizing:"border-box",background:"#0a0a0a",border:`1px solid ${manualCode?T.orange:T.border}`,
                padding:"9px 12px",color:T.white,fontSize:13,fontFamily:"monospace",outline:"none",borderRadius:8}}/>
            {manualCode.trim().length>3&&(
              <div style={{marginTop:6,fontSize:10,color:T.dim,letterSpacing:1}}>
                TIP: For best scan results, lay the bottle flat so the barcode faces up and fill the frame with just the barcode.
              </div>
            )}
          </div>
          {paints.length>0&&(
            <>
              <div style={{color:T.dim,fontSize:11,fontWeight:700,letterSpacing:1,textTransform:"uppercase",marginBottom:6}}>Search and link to an existing paint:</div>
              <PaintSearch allPaints={paints} onSelect={p=>{
                if(manualCode.trim()&&onTeach) onTeach(manualCode.trim(),p.id);
                onFound(p); onClose();
              }} brandFilter="all"/>
            </>
          )}
          {onAddNew&&<AddNewItemForm onAddNew={(item)=>{onAddNew({...item,barcode:manualCode.trim()||item.barcode});onClose();}} barcode={manualCode.trim()} mode={addNewMode}/>}
        </div>
      )}
      ...
      {status==="unknown"&&(
        <div style={{padding:"8px 0"}}>
          ...
          {/* Option 2 — add as new item */}
          {onAddNew&&<AddNewItemForm onAddNew={(item)=>{onAddNew({...item,barcode:scannedCode});onClose();}} barcode={scannedCode} mode={addNewMode}/>}
          <GhostBtn onClick={()=>setStatus("idle")} color={T.dim} style={{marginTop:8,width:"100%",justifyContent:"center"}}>TRY AGAIN</GhostBtn>
        </div>
      )}
    </div>
  );
