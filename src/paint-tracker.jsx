...
async function sbUpsert(table,rows,conflictCol="id"){
  ...
  try{
    ...
    console.log("sbUpsert",table,r.status,text.slice(0,100));
  ...
  }catch(e){console.error("sbUpsert error",table,e);_syncError=`POST ${table}: ${e.message}`;_syncSetStatus&&_syncSetStatus("error:"+_syncError);}
}
...
function PaintTrackerApp(){
  ...
  async function loadFromDB(){
    console.log("SYNC TRIGGERED", uid);
    setSyncStatus("loading");
    try{
      const testR=await fetch(`${_SB_URL}/pf_users?id=eq.${uid}`,{headers:{"apikey":_SB_KEY,"Authorization":"Bearer "+_SB_KEY}});
      if(!testR.ok){
        const t=await testR.text();
        setSyncStatus("error:"+testR.status+" "+t.slice(0,60));
        return;
      }
