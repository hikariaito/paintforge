...
function BarcodeScanner({allPaints,onFound,onTeach,onClose,onAddNew,addNewMode="shop"}){
  ...
  async function startLiveScan(){
    ...
    setCamError("");
    try{
      // Request camera first (this is what needs user gesture)
      ...
      // Load zxing-wasm after camera is running
      if(!window.ZXingWASM){
        await new Promise((res,rej)=>{
          const s=document.createElement("script");
          s.src="https://cdn.jsdelivr.net/npm/zxing-wasm@1.3.2/dist/full/zxing-wasm.js";
          s.onload=()=>setTimeout(res,800); // wait for WASM init
          s.onerror=(e)=>rej(new Error("Failed to load scanner library"));
      ...
      async function scanFrame(){
        ...
        try{
          ...
          if(zx&&zx.readBarcodesFromImageData){
            const results=await zx.readBarcodesFromImageData(imgData,{
      ...
      stopCamera();
      const msg=e?.name==="NotAllowedError"?"Camera permission denied — go to Settings > Safari > Camera > Allow."
        :e?.message?`Error: ${e.message}`
        :e?.name?`Error: ${e.name}`
        :"Camera could not be accessed. Try closing other apps using the camera.";
      setCamError(msg);
      setStatus("cam_error");
    }
  }
    ...
    <div style={{border:`1px solid ${T.border}`,background:"#0a0a0a",padding:12,borderRadius:10}}>
      ...
      {status==="idle"&&(
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {/* Live scan button — always shown, handles unsupported gracefully */}
          <button onClick={startLiveScan}
            style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,
              padding:"13px 0",background:T.orange,color:"#000",fontWeight:900,fontSize:12,
              letterSpacing:1,textTransform:"uppercase",borderRadius:10,cursor:"pointer",border:"none"}}>
          ...
          <label style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,
            ...
            letterSpacing:1,textTransform:"uppercase",borderRadius:10,cursor:"pointer",border:`1px solid ${T.border}`}}>
          ...
          <input ref={inputRef} value={manualCode}
            ...
            placeholder="Or type barcode / search by name..."
      ...
      {status==="starting"&&(
        <div style={{padding:"24px 0",textAlign:"center"}}>
          <div style={{color:T.orange,fontSize:12,fontWeight:900,letterSpacing:2,textTransform:"uppercase"}}>STARTING CAMERA...</div>
        </div>
      )}
        ...
        <div>
          <div style={{position:"relative",width:"100%",borderRadius:10,overflow:"hidden",background:"#000",aspectRatio:"4/3"}}>
            ...
            {[["top","left"],["top","right"],["bottom","left"],["bottom","right"]].map(([v,h],i)=>(
              <div key={i} style={{position:"absolute",[v]:12,[h]:12,width:24,height:24,
                [`border${v[0].toUpperCase()+v.slice(1)}`]:`3px solid ${T.orange}`,
                [`border${h[0].toUpperCase()+h.slice(1)}`]:`3px solid ${T.orange}`}}/>
            ))}
            <div style={{position:"absolute",bottom:8,left:0,right:0,textAlign:"center",
              color:T.orange,fontSize:10,fontWeight:900,letterSpacing:2,textTransform:"uppercase",
              textShadow:"0 1px 4px #000"}}>AIM AT BARCODE</div>
          </div>
      ...
      {status==="cam_error"&&(
        <div style={{padding:"8px 0",textAlign:"center"}}>
          <Badge color={T.red}>CAMERA UNAVAILABLE</Badge>
          <div style={{color:T.dim,fontSize:12,margin:"10px 0"}}>{camError}</div>
          <label style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,
            padding:"12px 0",background:T.orange,color:"#000",fontWeight:900,fontSize:12,
            letterSpacing:1,textTransform:"uppercase",borderRadius:10,cursor:"pointer",marginBottom:8}}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"/><circle cx="12" cy="13" r="4"/></svg>
            TAKE PHOTO OF BARCODE
            <input type="file" accept="image/*" capture="environment"
              onChange={e=>{setStatus("idle");decodeImage(e.target.files?.[0]);}} style={{display:"none"}}/>
          </label>
      ...
      {status==="decoding"&&(
        <div style={{padding:"24px 0",textAlign:"center"}}>
          ...
          <div style={{color:T.dim,fontSize:11,marginTop:6}}>Trying multiple methods</div>
        </div>
      )}
      ...
      {status==="photo_fail"&&(
        <div style={{padding:"8px 0"}}>
          ...
          <div style={{color:T.dim,fontSize:12,margin:"8px 0"}}>Couldn't read automatically.</div>
          {failedImage&&(
            <div style={{marginBottom:10,borderRadius:8,overflow:"hidden",border:`1px solid ${T.border}`,background:"#000"}}>
